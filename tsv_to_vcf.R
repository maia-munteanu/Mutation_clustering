...
file=paste0(sample,".snv.filt.random.R",niter,".vcf")
...
if(identical(original_tsv[,c(1,3,4,5)],random_tsv[,c(1,3,6,7)])){ #check that chr, pos, ref and alt fields for the original variants are the same
      ...
      write.table(c("##fileformat=VCFv4.2"), file, row.names = F,col.names = F,quote = F)
      fwrite(vcf, file, row.names = F,col.names = T,quote = F,append = T,sep="\t")
